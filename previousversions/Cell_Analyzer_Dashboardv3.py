...
# ==============================================================================
# INTERACTIVE BATCH DASHBOARD WITH MANUAL SLIDER
# ==============================================================================
class AdvancedBatchCellAnalyzer(ctk.CTk):
    def __init__(self):
        super().__init__()
        
        self.title("Zhang Lab | Manual Threshold Membrane Kinetics Dashboard")
    ...
    def setup_ui_layout(self):
        ...
        lbl_title = ctk.CTkLabel(self.sidebar, text="Advanced Batch Controls", font=ctk.CTkFont(size=16, weight="bold"))
        ...
        lbl_phase.pack(pady=(10, 2), padx=20, anchor="w")
        ...
        # --- NEW LIVE THRESHOLD SLIDER CONTROLS ---
        self.lbl_slider_title = ctk.CTkLabel(self.sidebar, text="Local Ring Threshold: 25%", font=ctk.CTkFont(weight="bold"))
        self.lbl_slider_title.pack(pady=(15, 2), padx=20, anchor="w")
        
        self.thresh_slider = ctk.CTkSlider(self.sidebar, from_=0, to=95, number_of_steps=19, command=self.on_slider_value_changed)
        self.thresh_slider.pack(pady=(0, 10), padx=20, fill="x")
        self.thresh_slider.set(25) # Default 25% drop limit cuts low background noise cleanly
        ...
        self.btn_clear_current.pack(pady=(15, 5), padx=20, fill="x")
        ...
        self.btn_refresh = ctk.CTkButton(self.sidebar, text="🔄 Refresh & Update Plots", fg_color="#2980B9", hover_color="#3498DB", command=self.render_global_comparison_chart)
        self.btn_refresh.pack(pady=(15, 5), padx=20, fill="x")
        ...
        self.btn_export_plots.pack(pady=5, padx=20, fill="x")

        # PANEL 2: CENTER WORKSPACE CANVAS
        self.canvas_frame = ctk.CTkFrame(self)
        self.canvas_frame.grid(row=0, column=1, sticky="nsew", padx=10, pady=10)
        self.lbl_canvas_status = ctk.CTkLabel(self.canvas_frame, text="Select data directory to initialize workspace.", font=ctk.CTkFont(slant="italic"))
    ...
    def load_flat_data_folder(self):
        ...
        if not valid_prefixes:
            tk.messagebox.showerror("Error", "No valid structural configurations found.")
            return
    ...
    # ==============================================================================
    # DYNAMIC REGION SELECTION & LIVE SLIDER RE-THRESHOLDING
    # ==============================================================================
    def on_lasso_release(self, verts):
        ...
        # 1. Log the absolute manual perimeter coordinate space
        ...
        # Append placeholder structures to initialize memory slot indexes
        self.raw_hand_masks[phase].append(raw_hand_mask)
        self.patches[phase].append(poly)
        self.masks[phase].append(raw_hand_mask) # Will be overwritten by slider filter instantly
        self.scatter_artists[phase].append(None) # Will be populated by slider draw instantly
        
        # Trigger the threshold filter on this new mask slot index immediately
        active_index = len(self.raw_hand_masks[phase]) - 1
        self.apply_threshold_to_slot(phase, active_index)

    def on_slider_value_changed(self, value):
        pct = int(value)
        self.lbl_slider_title.configure(text=f"Local Ring Threshold: {pct}%")
        
        # Re-filter EVERY drawn shape in the active workspace category whenever slider moves
        phase = self.phase_var.get()
    ...
    def apply_threshold_to_slot(self, phase, index):
        raw_hand_mask = self.raw_hand_masks[phase][index]
        color = 'r' if phase == "TREATED" else 'b' if phase == "CONTROL" else 'c'
        
        # Erase old overlay scatter artist points if they exist in this slot index
        ...
        final_mask = raw_hand_mask
        
        # Apply slider-percentage boundary limits to cell classes
        if phase in ["TREATED", "CONTROL"]:
            local_pixels = self.mcherry_stack[0][raw_hand_mask]
            if local_pixels.size > 0:
                mx = np.max(local_pixels)
                mn = np.min(local_pixels)
                # Calculate threshold line based on slider value
                thresh_line = mn + (self.thresh_slider.get() / 100.0) * (mx - mn)
                
                intensity_mask = self.mcherry_stack[0] >= thresh_line
                candidate_mask = np.logical_and(raw_hand_mask, intensity_mask)
                
                if np.sum(candidate_mask) > 0:
                    final_mask = candidate_mask

        # Log filtered array map mask to active processing index storage
        self.masks[phase][index] = final_mask
        
        # Re-draw the bright visual overlay dots matching new slider coordinates
        y_idx, x_idx = np.where(final_mask)
        if y_idx.size > 0:
            scatter_art = self.ax_canvas.scatter(x_idx, y_idx, color=color, s=1, alpha=0.4)
    ...
    # ==============================================================================
    # CORE PIPELINE PIPELINE LIFECYCLES
    # ==============================================================================
    def process_and_advance_queue(self):
        ...
        try:
            ...
            tk.messagebox.showerror("Spreadsheet Lock Error", f"Could not save metrics:\n\n{file_error}")
            return
    ...
    def generate_plots_engine(self, dark_mode=True):
        ...
        # CHART 1: GFP Connect-the-Dots
        ax1 = plt.subplot2grid((2, 2), (0, 0))
        ax1.set_facecolor(face_col)
        if not gfp_df.empty:
            for cond, color_line in [("Normoxia", "#5CB85C"), ("Hypoxia", "#2E7D32")]:
                sub = gfp_df[gfp_df["Condition"] == cond].sort_values('Wavelength_Num')
                if not sub.empty:
                    ax1.plot(sub['Wavelength_Num'], sub['Decay_Constant_k'], color=color_line, linestyle='-', marker=marker_dict.get(cond, "o"), markersize=8, linewidth=2, label=f"GFP {cond}")
        ...
        # CHART 2: mCherry Connect-the-Dots
        ax2 = plt.subplot2grid((2, 2), (0, 1))
        ax2.set_facecolor(face_col)
        if not mch_df.empty:
            for cond, color_line in [("Normoxia", "#D9534F"), ("Hypoxia", "#C0392B")]:
                sub = mch_df[mch_df["Condition"] == cond].sort_values('Wavelength_Num')
                if not sub.empty:
                    ax2.plot(sub['Wavelength_Num'], sub['Decay_Constant_k'], color=color_line, linestyle='-', marker=marker_dict.get(cond, "o"), markersize=8, linewidth=2, label=f"mCherry {cond}")
        ...
        for (fl, cond), grp in df.groupby(["Fluorophore", "Condition"]):
            ...
            if not grp.empty:
                ...
                ax3.plot(grp['Wavelength_Num'], grp['Decay_Constant_k'], color=c, linestyle='-', marker=marker_dict.get(cond, "o"), markersize=8, linewidth=2, label=f"{fl} [{cond}]")
                
        ax3.set_title("Unified Cross-Matrix Footprint Summary (Connected Series)", color=txt_col, fontsize=11)
    ...
    def export_publication_plots(self):
        ...
        for cond, col in [("Normoxia", "#5CB85C"), ("Hypoxia", "#2E7D32")]:
            sub = gfp_df[gfp_df["Condition"] == cond].sort_values('Wavelength_Num')
            if not sub.empty:
                ax1.plot(sub['Wavelength_Num'], sub['Decay_Constant_k'], color=col, linestyle='-', marker=marker_dict.get(cond, "o"), markersize=8, linewidth=2, label=f"GFP {cond}")
        ax1.set_title("GFP Photobleaching Spectral Response"); ax1.set_xlabel("Wavelength (nm)"); ax1.set_ylabel("k (s^-1)"); ax1.grid(True); ax1.legend()
        ...
        for cond, col in [("Normoxia", "#D9534F"), ("Hypoxia", "#C0392B")]:
            sub = mch_df[mch_df["Condition"] == cond].sort_values('Wavelength_Num')
            if not sub.empty:
                ax2.plot(sub['Wavelength_Num'], sub['Decay_Constant_k'], color=col, linestyle='-', marker=marker_dict.get(cond, "o"), markersize=8, linewidth=2, label=f"mCherry {cond}")
        ...
        for (fl, cond), grp in df.groupby(["Fluorophore", "Condition"]):
            ...
            if not grp.empty:
                grp = grp.sort_values(by='Wavelength_Num')
                ax3.plot(grp['Wavelength_Num'], grp['Decay_Constant_k'], color=c, linestyle='-', marker=marker_dict.get(cond, "o"), markersize=8, linewidth=2, label=f"{fl} [{cond}]")
        ax3.set_title("Unified Spectral Connected Matrix"); ax3.set_xlabel("Wavelength (nm)"); ax3.set_ylabel("Decay Rate Constant k (s^-1)"); ax3.grid(True); ax3.legend()
        ...
        tk.messagebox.showinfo("Export Complete", "Saved 3 point-to-point publication figures inside your workspace folder!")
