...
class AdvancedBatchCellAnalyzer(ctk.CTk):
    ...
    def initialize_dataset_at_index(self, index):
        ...
        power_match = re.search(r'(\d+(?:\.\d+)?)\s*m?W\b', prefix, re.IGNORECASE)
        if power_match:
            detected_power = power_match.group(1)
